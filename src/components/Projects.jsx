...
const Projects = () => {
    ...
    const [cardsToShow, serCardsTOShow] = useState(1);
    useEffect(() => {
        const updateCardsToShow = () => {
            if (window.innerWidth >= 1024) {
                serCardsTOShow(projectsData.length);
            } else {
                serCardsTOShow(1)
            };
        };
    ...
    lg:px-32 my-20 w-full overflow-hidden' id='Projects'>
            ...
            <div className='overflow-hidden'>
                ...
                easy-in-out'style={{ transform: `translateX(-${(currentIndex * 100) / cardsToShow}%)` }}
                >
                    {projectsData.map((project, index) => (
                        <div key={index} className='relative flex-shrink-0 w-full sm:w-1/4 '>
                            <img src={project.Image} alt={project.tittle} className='w-full h-auto mb-14' />
                            <div className='absolute left-0 right-0 bottom-5 flex justify-center'>
                                <div className='inline-block bg-white w-3/4 px-4 py-2 shadow-md'>
                                    <h2 className='text-xl font-semibold text-gray-800'>{project.tittle}</h2>
